...
import { getZonedDateInfo } from './date.js';
...
describe('getZonedDateInfo', () => {
  it('returns correctly parsed date info for a valid date and timezone', () => {
    ...
    expect(result.isWeekendCheck).toBe(true);
    expect(result.daysInCurrentMonth).toBe(31);
    expect(result.timestamp).toBe(new Date('2023-01-15T12:00:00Z').getTime());
  });

  it('returns null for an invalid date', () => {
    const result = getZonedDateInfo('invalid-date', 'America/New_York');
    expect(result).toBeNull();
  });

  it('returns null for an invalid timezone string', () => {
    const result = getZonedDateInfo('2023-01-15T12:00:00Z', 'Invalid/Timezone');
    expect(result).toBeNull();
  });
});
